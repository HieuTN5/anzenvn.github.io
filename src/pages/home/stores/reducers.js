import { SAVE_ALL_ORDERS, PI_LOADING_REQUEST, PI_LOADING_SUCCESS, SAVE_INFO_ORDER } from './constants';
import initialState from './state';

const reducers = {
  [SAVE_ALL_ORDERS](state, { payload }) {
    const { data: currentState } = state;
    // const newPayload = payload.map((item, index) => ({ ...item, key: index + 1 })) || [];
    currentState.dataOrders = payload;
  },
  ...
  [SAVE_INFO_ORDER](state, { payload }) {
    const { data: currentState } = state;
    currentState.infOrder = payload;
  },
};
